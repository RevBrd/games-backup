...
| [Shadowless](Shadowless/) | The whole WotC-era Pokémon TCG — every card, played to the letter of the original ruleset. Win matches, win packs, build decks from what you actually own, fill a dex | The biggest project here by far, and the only one with a build step: **`shadowless.html` is generated from `src/`, so never hand-edit it.** Jobs 1–16 are done and five sets are live at 442 of 442 cards — Base, Jungle, Fossil, Team Rocket and Gym Heroes — plus a **Challenge bracket that belongs to no set**, a round of seven mono-type opponents sitting between two of them and paying in a pack of its own. Every rung on all six brackets is a deck Trevor hand-built or an authentic theme deck. Packs of whichever set you are on, cosmetic variants, a dex and a deck builder, all persisted. Six suites behind one gate — `node tools/test.js`, which runs the generator `--check`s first because the suite that tests the built file will happily pass against a build you forgot to make — plus a screenshotter, because the smoke stub has no layout engine. **Read its `CLAUDE.md` first** — the depth is in a sibling per subject plus three directories, `Rulings/`, `Playbook/` and `AI-INVARIANTS/` — never counted in prose, because that count rots every time an archive rolls over, and that tree is the thing most worth stealing from this repo: `MAINTENANCE.md` is how a doc tree gets un-drifted and `tools/doccheck.js` is the half of it a machine can run — built after four consecutive passes missed the same written instruction, `PLAYTEST.md` is how to work a bug report from a human, and `HISTORY.md` keeps rejected ideas with the reason each one lost |
