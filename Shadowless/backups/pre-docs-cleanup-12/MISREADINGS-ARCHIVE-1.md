# Shadowless — how a measurement lied, archive 1 (11–26 Aug 2026)

**The first archive of [MISREADINGS.md](MISREADINGS.md).** Split on 2 Sep 2026 at the end of Job 15d,
when the live file reached 478 against the ~450 threshold in its own header — a threshold written that
same morning and blown by that same afternoon's four entries.

**Everything here is still true and still worth reading.** These are the oldest entries, 11–26 Aug
2026, and several are the most-cited lessons in the tree: the counter that says "must be 0" and is
counting the wrong thing, the harness that never dealt the situation and reported 50%, the two months
every scripted game ran at 12 Prizes. [MEASUREMENT.md](MEASUREMENT.md) keeps the one-line shape of
each; the full accounts are here.
...
**Start `MISREADINGS-ARCHIVE-2.md` rather than growing this one past ~250**, and the live file's
header carries that rule too, because a constraint stated only in the thing being constrained is read
too late.

**No positional references survive here.** Every "the entry above" and "two entries up" in the source
material was rewritten to name its entry before the split, per `MAINTENANCE.md` — a split re-points
those at different material silently rather than breaking loudly.
...
at all about the three sets it does not contain. It is the same shape as the two `--gbc` entries (`aiduel.js` on four theme decks, and `aitest.js` blind the same way) —
