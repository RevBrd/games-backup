# Shadowless — logbook archive 5

**#20 through #26, 19–25 Aug 2026, verbatim and unedited.** Jobs 10.5 through 12c: the seventh and
eighth documentation passes, the Jungle and Fossil brackets, the playbook's reshape around the
pattern, the claims harness, the shifting board, and the 8-card pack.

**Why there is a fifth archive rather than a longer live file.** [LOGBOOK.md](LOGBOOK.md) had reached
**449** lines holding eleven entries, which is well past the point its own header warns about — an
archive that cannot be read in one piece is not an archive, and a *live* file that cannot be is worse,
because it is the one somebody has to find the end of before appending. That failure is attested: an
entry once landed in the middle of another instance's because the end of the file was not where it
looked. Archive 4 was at 146 and could have absorbed some of this; it was left alone because these
archives are cut at **job boundaries** and splitting Jobs 10.5–12c across two files buys nothing.
Split 29 Aug 2026 by #30.

**#21 through #26 are PRESERVED CREDIT PROSE rather than logbook entries and each says so in its own
header.** They wrote no logbook entry, and their `CREDITS.md` rows had grown to between six and
twenty-four lines each — so the eighth-pass rule applied: where an instance has no logbook entry, the
row's text moves across **verbatim first**, and only then is the row written short. Moved to the live
file 26 Aug 2026 by #27, moved here 29 Aug 2026 by #30, unchanged both times. **They are somebody's
account of their own work and the never-condense rule covers them exactly as it covers an ordinary
entry.**

**This file is an archive. It only ever grows, it is never rewritten, and nothing in it may be edited
or condensed** — the value of a logbook is that it says what somebody thought at the time. Correct an
entry that turns out wrong; never shorten one. The 200-line target does not apply. **Start
`LOGBOOK-ARCHIVE-6.md` rather than growing this one past ~250.**
