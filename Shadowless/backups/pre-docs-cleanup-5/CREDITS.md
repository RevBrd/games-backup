...
words of whoever wrote it. The logbook is completely optional. A row with no logbook entry is fine. A logbook entry with no row is how
somebody gets left off. Sign your session number wherever you remember to. Trevor will attempt to label any that are forgotten as yours.
...
| **Opus 5** #9 | 11 Aug 2026 | The post-Job-6 documentation pass: this file and [LOGBOOK.md](LOGBOOK.md), the Job 6 collapse into [HISTORY.md](HISTORY.md), four corrections where a doc pointed at working code and called it a gap, and `miniCard()`'s retirement, event logger v1.0 |
...
| **Opus 5** #12 | 13-14 Aug 2026 | Maintenance from Trevor's playtest logs and [GRABBAG.md](GRABBAG.md). Confusion now flips on retreat and charges before it rolls, with the AI taught to price it. A deck name resolves to the deck deck select is offering — a blueprint and a built deck sharing the builder's default name had the tile drawing the wrong one and Play fielding a 41-card list. Then the AI: the retreat re-tune `AI.md` had been asking for since the 12-Prize repair, which turned out to be a curve rather than a number; Weakness and Resistance reaching the one forecast path that never called `computeDamage`; and DERIVED stickiness, so the bot leaves Kangaskhan and Snorlax in. Two harness faults found underneath all of it — `selftest.js`'s ladder gate asserting a statistical claim at a sample that could not carry it, and `aiduel.js` playing only the four Base Set decks, which made it blind to any change about the other five-sixths of the card pool. Then Arcanine: recoil priced on the HP it leaves you rather than flat, and overkill damage no longer paid for. And [PLAYTEST.md](PLAYTEST.md), the method file for working Trevor's grab bag, at his request |

| **Sonnet 5** #13 | 14 Aug 2026 | Opponent-deck research for the placeholder roster PROGRESSION.md flags as a future job. Found `data/Deck Lists/GB2 Opponent Deck Guide (Alamedyang, English-patch names).txt` — a hex-extracted, ~90-opponent deck guide for the Japan-only GBC2 sequel, the closest thing to a second `gbc_decks.json` source that exists. Pulled and ID-mapped 16 official WotC theme decks (Fossil, Base Set 2, Team Rocket, all 8 Gym Leaders) and 8 flavor-named GBC2 decks into `fossil_decks.json`, `base4_decks.json`, `team_rocket_decks.json`, `gym_decks.json` and `gbc2_flavor_decks.json` — all reference data, quarantined, nothing reads them yet. `data/OPPONENT_DECK_POOL.md` is the running tracker for picking from the pool as sets go live |
