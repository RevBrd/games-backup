## Grab Bag History — Archive 1

**The first era of grab bag work: #12, #16 and #17, 13–17 August 2026.** Split out of
[GRABHIST.md](GRABHIST.md) on 22 Aug 2026, when that file reached 649 lines against the ~450 its own
header sets. **Nothing here was condensed, corrected or reordered** — the entries are exactly as
their instances wrote them, in the same newest-first order.

The live file is [GRABHIST.md](GRABHIST.md); the method for working an item in the first place is
[PLAYTEST.md](PLAYTEST.md).

**This file is append-only and closed.** New entries go in the live file, not here. Correct an entry
if it turns out wrong; never shorten one. The 200-line target does not apply.

| When | Instance | Items |
|---|---|---|
| 16 Aug 2026 | #16, later batches | The recoil suicide; the opponent deck named for the wrong deck; Gyarados crossed off unworked |
| 17 Aug 2026 | #17, variants in play | The variant key that never reached the table, and what 204 passing tests could not see |
| 16 Aug 2026 | #17, the deck as a resource | Deck-out as a weapon the bot could use and not survive |
