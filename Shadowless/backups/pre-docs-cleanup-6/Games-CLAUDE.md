...
| [Shadowless](Shadowless/) | The whole WotC-era Pokémon TCG — every card, played to the letter of the original ruleset. Win matches, win packs, build decks from what you actually own, fill a dex | The biggest project here by far, and the only one with a build step: `shadowless.html` is **generated** from `src/`, so never hand-edit it. **Jobs 6 and 7 are done and three sets are live — Base, Jungle and Fossil, all 221 cards:** work down a ladder of named challengers, beat the rival at the end of a bracket to open the next set, earn booster packs of whichever set you are on, open them card by card, browse your collection and dex, and build decks from what you own, all persisted to a save you can export. Cards can roll cosmetic variants (Shiny, Reverse Holo, 1st Edition, Shadowless, Misprint) and the deck builder lets you choose which physical copy goes in. Six test suites, ~550 assertions, and `tools/shot.js` screenshots the built game at any viewport — the smoke stub has no layout engine, so looking at it is the only test for a whole class of bug, and Job 7b is the sharpest case yet: four layout defects were live while all 136 smoke tests passed. The ladder's brackets are DERIVED from the live-set list, so adding a set is adding a set rather than rewiring progression. **Job 8 is done, and it is a design job rather than a build:** `OPPONENTS.md` specifies what the placeholder opponents eventually become — four silent difficulty tiers with on-brand labels mapped on top later, one entry-condition mechanism behind three different deck gates, and a pressure tag saying what a deck DOES to you rather than how strong it is. Read its `CLAUDE.md` before anything; the depth lives in twenty-one siblings plus a `Rulings/` folder — **`RULINGS.md` is now a directory over one-file-per-ruling**, worth stealing for any append-only register that outgrew its file — six of them deliberately reached from their parent rather than from the index, including an `AI.md` for the one part of the game no test suite can judge, a `PLAYTEST.md` for working the running list of things Trevor notices while playing -- where the report has three times out of three not meant what it said, a `HISTORY.md` that holds rejected ideas with the reason each one lost, and a `LOGBOOK.md` that is append-only. **Job 9's first batch is in** — an alpha-testing pass over Trevor's playtest notes, and the generalisable half is that three of the five changes were to the *match log* rather than to the game: proving one report wrong needed a pass that says why it did not attack, an opening board that setup never recorded, and a way to tell two identical Squirtle apart. Worth stealing for anything with a human reporting bugs at it |
...
- Almost every game in this catalog was named by the instance that created it, with the exceptions of GemTD, Blocks ig, Snek, and Ultra Pong. Please continue naming them yourself upon file creation. Situations where that will not be the case are exceedingly rare.
...
## Games planned to ship in the first upload batch when all are complete:

DRIFT, Dead Space: Block Party, Ultra Pong, Afterglow, Shadowless if it's free-only and liability-safe. Others will ship upon their own completion.
