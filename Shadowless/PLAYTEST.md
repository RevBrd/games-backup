...
`GRABBAG.md` and what came of each item is [GRABHIST.md](GRABHIST.md).

Written 14 Aug 2026 by the instance that worked the first batch, at Trevor's request, for whoever
...
**A log predates your build.** Trevor plays while you work, so the file may be from before the change
...
English, and `CLAUDE.md` names that as the project's arbiter for ambiguous card text.

**He answers design questions as decisions, not preferences.** "A confused Pokémon pays the Energy
before it flips" arrived in one sentence and became a ruling. When an answer is his, say so where you
record it — [RULINGS.md](RULINGS.md) marks those *settled with Trevor*, and it changes how much
authority a later instance has to overturn them.
