...
- The AI will not build a big Colorless attack one basic Energy at a time — **your read was right about what you saw and the mechanism is one step over.** log# 04-26-10. Nothing avoids non-DCE Energy by type: every basic type scores identically on a Colorless Pokemon, measured. What happens is that `potential()`'s `short` is the distance to the CHEAPEST attack the card can reach, so it pins at 0 the moment any attack is payable — and the attach rule then reads "no progress" and refuses. Chansey with two Energy scores a basic at **−2** and a Double Colorless at **185**: it can reach Scrunch, so it is treated as finished, and it can never walk up to Double-edge. Same for anything with a cheap first attack and an expensive second one that does not scale. **A design question rather than a bug** — how much is an Energy toward an attack two turns away worth? — so it is left here for you. The two Chansey retreats in that log both set up attacks and one of them won a Prize; they look right. → [GRABHIST](GRABHIST.md)
