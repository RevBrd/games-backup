# Shadowless — logbook archive 8: Job 15d and Job 15e

**#35, #36 and #37, verbatim.** Job 15d (the suite audit, `owed.js`, and the gate), and Job 15e in
its two halves — the GBC 2 seam and Potion timing, then the attack road and `destShort`. Moved here
on 8 Sep 2026 by Shadowless 39 at the start of Job 15g, when the live file had reached **555** lines
against the ~450 rule in its own header. That is the fourth consecutive documentation pass to find a
register over the limit written at the top of its own file, which is why the pass now runs
`node tools/doccheck.js` instead of remembering to look.
...
actually thought at the time, and a condensed account keeps the conclusion and loses the reasoning.

**Start `LOGBOOK-ARCHIVE-9.md` rather than growing this one past ~450.** An archive's limit is larger
than the live file's because the live limit guards against two instances appending into each other,
which cannot happen in a closed file — Trevor's call, 2 Sep 2026.
