    ...
    for (const v of script) {
      switch (v.v) {
        ...
        case 'T_ENERGY_SEARCH':
          // ANY Energy card, not only a basic one. See Rulings/ENERGY-SEARCH.md.
          if (!p.deck.some(x => this.db[x.id].kind === 'energy')) return false;
          break;
  ...
  doTrainer(pi, a) {
    ...
    for (const v of script) {
      switch (v.v) {
        ...
        case 'T_ENERGY_SEARCH': {
          // "Search your deck for a basic Energy card" — and the Game Boy game
          // let you take a Double Colorless anyway. Trevor's call, 18 Aug 2026,
          // and it is the whole reason Charizard is a workable deck there.
          // Deliberately NOT the printed text. See Rulings/ENERGY-SEARCH.md.
          const eligible = p.deck.map((x, i) => [x, i])
            .filter(([x]) => this.db[x.id].kind === 'energy');
          if (!eligible.length) { this.log('Energy Search: no Energy left.'); this.shuffle(p.deck); break; }
          let at = -1;
          if (a.opts && a.opts.pickUid !== undefined) at = p.deck.findIndex(x => x.uid === a.opts.pickUid);
          if (at === -1) at = eligible[this.pick(eligible.length)][1];
          const got = p.deck.splice(at, 1)[0];
