...
class AI {
  ...
  evolutionRoadFor(pi, slot) {
    ...
    const contenders = twins.filter(x => this.potentialAs(pi, x, evo, null).short > 0);
  ...
  attachValue(pi, slot, energyId) {
    ...
    if (afterR.short === 0 && beforeR.short > 0 && !evo) {
      ...
      s += W.attachBuild * beforeR.short * 2;
    } else if (afterR.short < beforeR.short) {
      const steps = beforeR.short - afterR.short;
      s += Math.max(10, beforeR.goal) * (steps / beforeR.short)
         * W.attachAmortise * this.survivesCharge(pi, slot, afterR.short);
  ...
  scoreAction(pi, a) {
    ...
    switch (a.t) {
      ...
      case 'attachEnergy': {
        ...
        // stops the moment the evolved form could pay for its cheapest attack.
        // That is Trevor's "get energies close to that point before actually
        // evolving", and it composes with the readiness term in `case 'evolve'` —
        // that one fires at one-short, so the card evolves a turn before this
        // exception would have closed and attaches onto the evolved form instead.
        const evoId = this.evolutionRoadFor(pi, slot);
        const evolving = !!evoId && this.potentialAs(pi, slot, evoId, null).short > 0;
      ...
      case 'evolve': {
        ...
        //
        // "Cheapest attack of value" is `potentialOf().short`, which already means
        // exactly that: symbols short of the cheapest attack this card can reach,
        // ties broken by damage. No new definition, and no threshold anybody had
        // to invent.
        //
        ...
        const readiness = this.potentialOf(pi, slot, newC).short;
      ...
      case 'retreat': {
        ...
        if (delta > 0) {
          const wall = wallScore(E.db, this.eff, this.top(me.active));
      ...
      switch (v.v) {
        ...
        case 'T_PLUSPOWER': {
          ...
          if (turnsWith < turnsNow) {
            ...
            s += this.incomingThreat(pi) >= this.remainingHP(me.active) ? 6 : 0.2;
          }
          break;
        }
