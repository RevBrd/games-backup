...
//                                  (Hyper Flame)
//     ENERGY_FROM_DISCARD_TO_SELF {n, t}   `t` narrows to one basic type (Lt.
//                                  Surge's Charge). Absent means any Energy
...
//                                  during your opponent's next turn. `sure` is the
//                                  UNCONDITIONAL printing - 10 across 4 sets
...
//     DISCARD_DEF_ENERGY {flip}    discard 1 Energy card from the defender. `flip`
//                                  is Removal Pulse: the coin is thrown ONLY if the
//                                  defender is holding Energy, which Sabrina's ESP
//                                  depends on - it counts coins actually thrown
...
const EFFECTS = {
  ...
  // Card 131, and the line that turns Gym Heroes live. Landed on its own once
  // Trevor's roster was in, 14 Sep 2026. See Rulings/RECALL.md.
  'gym1-116': { t: [{ v: 'T_RECALL' }] },                  // Recall
  'gym1-117': { t: [{ v: 'T_ESP' }] },
  ...
  // share an attack.
  //
  // SIX CARDS ARE DELIBERATELY ABSENT FROM THIS BATCH and they all failed the
  // same way: gym1-2 Brock's Rhydon, gym1-26 Erika's Victreebel, gym1-29 Misty's
  // Cloyster, gym1-42 Erika's Dratini, gym1-47 Erika's Oddish, gym1-65 Blaine's
  // Vulpix. Every one has its attacks covered by the copy above AND CARRIES A
  // POKEMON POWER, which the text-matching pass never looked at.
  //
  // They shipped like that for one commit. An `a:` with no `p:` is a card the
  // deck validator accepts and whose Power silently does nothing — the failure
  // that validator exists to prevent, one level in, and worse than an unscored
  // verb because the HUMAN loses the card too. selftest.js now asserts it.
  //
  // Do not re-add them from the attack text. They are a POWERS.md job.
  ...
  // Derived from printed text by anchored pattern rather than by copying another
  // card — every parameter below is read out of the sentence beside it. These four
  // carry NO Pokemon Power, checked explicitly, which is the thing the batch above
  // got wrong.
  'gym1-49': { a: [[{ v: 'STATUS', s: 'Asleep' }, { v: 'STATUS', s: 'Poisoned' }], []] },   // Erika's Weepinbell: Sleep Poison; Vine Whip (plain)
  'gym1-76': { a: [[{ v: 'RECOIL', n: 10 }]] },                        // Erika's Bellsprout: Careless Tackle
  'gym1-87': { a: [[{ v: 'STATUS_COIN_EITHER', heads: 'Paralyzed', tails: 'Asleep' }],
                   [{ v: 'DMG_PER_HEAD', coins: 2, per: 20 }]] },      // Misty's Poliwag: Hypnotic Stare; Tail Rap
  'gym1-95': { a: [[{ v: 'STATUS_SELF', s: 'Asleep' }]] },             // Sabrina's Slowpoke: Lazy Attack
  'gym1-92': { a: [
  ...
  // for this batch, which is the criterion it was selected on.
  //
  // AND EVERY ONE OF THEM WAS CHECKED FOR A POKEMON POWER before it went in.
  // The last batch derived scripts from attack text and shipped six cards whose
  // Power line nobody read; selftest now guards that, but the guard is the
  // backstop and reading the card is the method.
  'gym1-7':  { a: [[{ v: 'DMG_PER_HEAD', coins: 5, per: 10 }],
                   [{ v: 'CANT_RETREAT_ON_FLIP', sure: true, label: 'Clutch' }]] },
  'gym1-72': { a: [[{ v: 'CANT_RETREAT_ON_FLIP', sure: true, label: 'Sand Pit' }]] },
  'gym1-93': { a: [[{ v: 'CANT_RETREAT_ON_FLIP', sure: true, label: 'Spook' }]] },
  'gym1-50': { a: [[{ v: 'DISCARD_DEF_ENERGY', flip: true, label: 'Removal Pulse' }],
                   [{ v: 'STATUS_ON_FLIP', s: 'Confused' }]] },
  'gym1-96': { a: [[{ v: 'STATUS', s: 'Poisoned' }],
                   [{ v: 'DISCARD_DEF_ENERGY', flip: true, label: 'Removal Beam' }]] },
  // Horn Toss and Shake are the same attack on two cards, and both are WHIRLWIND
  // with damage: the printed "he or she chooses" is the defending player's
  // choice, which is what separates this from SWITCH_DEFENDER_CHOOSE one line
  // below. "(Do the damage before switching)" is the engine's order already.
  'gym1-22': { a: [[{ v: 'WHIRLWIND' }], [{ v: 'RECOIL', n: 10 }]] },
  'gym1-35': { a: [[{ v: 'WHIRLWIND' }], [{ v: 'RECOIL', n: 10 }]] },
  'gym1-28': { a: [[], [{ v: 'COST_DISCARD_ALL_ENERGY' }]] },
  // One-Two Punch has NO recoil, so the verb runs with `recoil` absent — one
  // coin, one consequence, which is the degenerate case of a verb built for
  // cards that hang several off the same flip.
  'gym1-36': { a: [[{ v: 'FLIP_OR_NOTHING' }],
                   [{ v: 'FLIP_BONUS_OR_RECOIL', base: 30, bonus: 10 }]] },
  // "Both ... are now Asleep (after doing damage)" is two verbs rather than one
  // because they are genuinely independent: a Barrier stops the defender's half
  // and cannot stop ours. The parenthetical is the engine's ordering already.
  'gym1-46': { a: [[{ v: 'STATUS', s: 'Asleep' }, { v: 'STATUS_SELF', s: 'Asleep' }],
                   [{ v: 'STATUS', s: 'Confused' }, { v: 'STATUS_SELF', s: 'Confused' }]] },
  'gym1-48': { a: [[], [{ v: 'REQUIRE_OPP_BENCH' }, { v: 'SWITCH_DEFENDER_FIRST' }]] },
  // Taunt does NOT say "can't be used" where Flytrap above does, so it is legal
  // into an empty Bench and quietly does nothing. The cards draw that
  // distinction themselves and printed text is step 1 of the rulings order.
  'gym1-67': { a: [[{ v: 'SWITCH_DEFENDER_CHOOSE' }], []] },
  'gym1-84': { a: [[{ v: 'DMG_PER_HEAD', coins: 1, per: 20 }],
                   [{ v: 'DMG_PER_HEAD', coins: 2, per: 20 }]] },
  // Defense Curl prevents DAMAGE only; Agility prevents all effects. Two verbs,
  // and the cards say which is which in so many words.
  'gym1-71': { a: [[{ v: 'PREVENT_ALL_DMG_SELF_ON_FLIP' }], []] },
  'gym1-63': { a: [[{ v: 'BARRIER_ON_FLIP', label: 'Agility' }]] },
  'gym1-81': { a: [[{ v: 'ENERGY_FROM_DISCARD_TO_SELF', n: 1, t: 'L' }],
                   [{ v: 'STATUS_ON_FLIP', s: 'Paralyzed' }]] },
  'gym1-78': { a: [[{ v: 'HEAL_SELF_IF_DAMAGED', n: 1 }],
                   [{ v: 'HEAL_SELF_ON_FLIP', n: 1, label: 'Sporadic Sponging' }]] },
