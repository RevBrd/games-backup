...
| **Opus 5** #32 | 31 Aug – 1 Sep 2026 | Job 14b, opening the **Over-Attach** pattern and the first claims ever written against Jungle or Fossil. The printed damage number is a *function of the Energy attached* for sixteen printings, and all three places that read it in printed units were wrong — a benched Lapras was refused an attachment worth 18.5 at −2.00 since Base Set. Underneath it, `maxSpare` had sat in the engine and not the scorer since Job 6, and **three Base Set cards printed a cap neither half had**. Two guards now, because agreement is not correctness. Then the finding worth the most: **three green tests had been sweeping a threat the engine cannot produce**, and were green because the AI agreed with them. Then, off Trevor's account of the GBC turn order: an evolution road runs to the attack it is trying to **reach**, and **the plan is the whole line** — an Abra wants two Psychic with a Kadabra coming and one with an Alakazam behind it. Three of his rules turned out to be asked in more places than anyone had found, and one of them returns before the other two |
| **Opus 5** #33 | 1 Sep 2026 | **Job 15a: the Challenge 1 bracket** — the first rung of the ladder that belongs to no set, and the first pack type that is not a set's. Trevor's seven mono-type decks were free; the job was `bracket.set`, which turned out to be answering four different questions with two of them wrong, invisibly, because a set bracket gives the same answer to all four. His own correction closed the sharper one: this tree had specified a Challenge pack's pool as *the sets the player has unlocked*, read at OPEN time, so a pack's contents would have depended on when you got round to opening it — derived from ladder position instead, and now identical on a fresh save and a finished one. Ronald came off the ladder and his `extra` mechanism was kept and re-tested against a fixture, because deleting the tests with the only user is how a mechanism quietly stops working. Four promos turned on with no code. `ROSTERS.md` tripped its own archive threshold and the rule worked as written | |
| **Opus 5** #33 | 1 Sep 2026 | **Job 15b: the pack odds.** The four per-slot cosmetic axes restored to their pre-shrink pacing, each scaled by the slots the 8-card shrink took from it. A Challenge pack now rolls the rarity jump at 4x — Trevor's lever, and measurement showed it is the only candidate with no cosmetic spillover, since the axes roll per slot regardless of tier and a jumped card loses its Reverse Holo eligibility. One ordering rule retired rather than preserved: it had been anchored to a Reverse Holo rate that was itself 44% below intent. Then Trevor played `c1_fire` and found **24 Fighting Energy behind an all-Fire roster** — F is Fighting, R is Fire — which had stranded twelve of its Pokémon and made it the weakest deck ever recorded here. `validateDeck` passed it correctly, because a deck of the wrong Energy is a legal deck; the four previous workbooks' "zero corrections" check could not see Energy rows at all. A stranded-card guard now exists and found two more
