...
| **Sonnet 5** | 14 Aug 2026 | Opponent-deck research for the placeholder roster PROGRESSION.md flags as a future job. Found `data/Deck Lists/GB2 Opponent Deck Guide (Alamedyang, English-patch names).txt` — a hex-extracted, ~90-opponent deck guide for the Japan-only GBC2 sequel, the closest thing to a second `gbc_decks.json` source that exists. Pulled and ID-mapped 16 official WotC theme decks (Fossil, Base Set 2, Team Rocket, all 8 Gym Leaders) and 8 flavor-named GBC2 decks into `fossil_decks.json`, `base4_decks.json`, `team_rocket_decks.json`, `gym_decks.json` and `gbc2_flavor_decks.json` — all reference data, quarantined, nothing reads them yet. `data/OPPONENT_DECK_POOL.md` is the running tracker for picking from the pool as sets go live |
