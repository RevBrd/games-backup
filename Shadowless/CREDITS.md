...
| **Opus 5** #43 | 18 Sep 2026 | **Job 17b, the doc half: `AI.md`'s Open list, and the register hiding in `selftest.js`.** The five silent-failure surfaces were rostered only inside the guard that watches them, so the model file described one and scattered four across its own closed items; they are a table in the body now, with `ENGINE.md` corrected where it still called one of them unguarded. Item 19's re-entry rule was cited by three invariant entries while living in a struck-through list item — promoted, and given the entry it never had. Closed items shrank to stubs against a written table of **which** home an account goes to, which is the part that was missing: three had stayed at full length because nobody had said where they should go. Items 7 and 9 closed and reopened on Trevor's answers. |
