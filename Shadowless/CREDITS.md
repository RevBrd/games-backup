...
| **Opus 5** #32 | 31 Aug – 1 Sep 2026 | Job 14b, opening the **Over-Attach** pattern and the first claims ever written against Jungle or Fossil. The printed damage number is a *function of the Energy attached* for sixteen printings, and all three places that read it in printed units were wrong — a benched Lapras was refused an attachment worth 18.5 at −2.00 since Base Set. Underneath it, `maxSpare` had sat in the engine and not the scorer since Job 6, and **three Base Set cards printed a cap neither half had**. Two guards now, because agreement is not correctness. Then the finding worth the most: **three green tests had been sweeping a threat the engine cannot produce**, and were green because the AI agreed with them |
